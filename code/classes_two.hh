...
class Graph { //currently storing globals in graph object
    public:
       ...
       ThreadSafeUO_map GDD;
        std::__1::chrono::steady_clock::time_point LAST_CHECK; //only works on mac
        //std::chrono::system_clock::time_point LAST_CHECK; //Only works on linux
        ...
        //void combine(std::map<std::string,std::set<int> > &local_count);
        //void set_p(int no_of_processors); the idea here is that we wouldn't want to
        //be locking if P = 1, but later on we can configure code s.t. it calls sequential 
        //code for P=1 so it doesn't really matter
